...
exports.buildFilters = (queryParams) => {
  ...
  if (is_paid && is_paid.length > 0) {
    const paidValue = is_paid[0].toLowerCase();
    if (['yes', 'true'].includes(paidValue)) {
      filters.push({ is_paid: true });
    } else if (['no', 'false'].includes(paidValue)) {
      filters.push({ is_paid: false });
    }
  }
